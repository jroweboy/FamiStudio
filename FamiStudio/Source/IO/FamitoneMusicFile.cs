...
namespace FamiStudio
{
    public class FamitoneMusicFile
    {
        ...
        private string ifndef = ".ifndef";
        private string def = ".define";
        private string fi = ".endif";
        ...
        private int OutputHeader(bool separateSongs)
        {
            ...
            {
                // When using EPSM, we allow the user to reduce the number of channels processed for a speed up.
                // This header just turns on all channels if they enabled EPSM
                lines.Add($"{iff} FAMISTUDIO_EXP_EPSM");
                lines.Add($"{ifndef} FAMISTUDIO_EXP_EPSM_SSG_CHN_CNT");
                lines.Add($"FAMISTUDIO_EXP_EPSM_SSG_CHN_CNT {def} 3");
                lines.Add($"{fi}");
                lines.Add($"{ifndef} FAMISTUDIO_EXP_EPSM_FM_CHN_CNT");
                lines.Add($"FAMISTUDIO_EXP_EPSM_FM_CHN_CNT {def} 6");
                lines.Add($"{fi}");
                for ( int i = 1; i <= 6; ++i)
                {
                    lines.Add($"{ifndef} FAMISTUDIO_EXP_EPSM_RHYTHM_CHN{i}_ENABLE");
                    lines.Add($"FAMISTUDIO_EXP_EPSM_RHYTHM_CHN{i}_ENABLE {def} 1");
                    lines.Add($"{fi}");
                }
                lines.Add($"{fi}");
            }
            ...
            for (int i = 0; i < project.Songs.Count; i++)
            {
                ...
                if (project.UsesEPSMExpansion)
                {
                    for (int chn = 0 ; chn < song.Channels.Length; ++chn)
                    {
                        ...
                        {
                            line += EpsmChannelDefine(channel) + "\n";
                        }
                        line += $"\t{dw} ";
        ...
        private List<string> GetSongData(Song song, int songIdx, int speedChannel)
        {
            ...
            for (int c = 0; c < song.Channels.Length; c++)
            {
                ...
                if (channel.IsEPSMChannel)
                {
                    songData.Add(EpsmChannelDefine(channel));
                }
        ...
        int CompressAndOutputSongData(List<string> data, int songIdx, int minNotesForJump, bool writeLines)
        {
            ...
            bool IsMacro(string str) => str.StartsWith(iff) || str.StartsWith(fi);
                    ...
                    if (compressible)
                    {
                        ...
                        if (compressible && patterns.TryGetValue(crc, out var matchingPatterns))
                        {
                            ...
                            foreach (var idx in matchingPatterns)
                            {
                                ...
                                {
                                    if (compressedData[j] != data[k] || IsLabelOrRef(compressedData[j]) || IsMacro(compressedData[j]))
                                        break;
            ...
            for (int i = 0; i < compressedData.Count; i++)
            {
                ...
                var isMacro = IsMacro(b);
                var isJumpCode = jumpToRefs.Contains(i);
                var isJumpLabel = IsRef(b);

                if (byteString != null && (isJumpLabel || isLabel || isMacro || isRef || isJumpCode || byteString.Length > 120))
                {
                    if (writeLines)
...
#if DEBUG
            var size = GetAsmFileSize(lines);
            Debug.Assert(size == headerSize + instSize + mappingsSize + tempoSize + totalSongsSize);
#endif
        ...
        private string EpsmChannelDefine(Channel channel)
        {
            var ssg_idx = channel.ExpansionChannelIndex;
            var fm_idx = channel.ExpansionChannelIndex - 3;
            var rhythm_idx = channel.ExpansionChannelIndex - 9 + 1;
            if (channel.IsEPSMSquareChannel)
            {
                return $"{iff} FAMISTUDIO_EXP_EPSM_SSG_CHN_CNT > {ssg_idx}";
            }
            else if (channel.IsEPSMFmChannel)
            {
                return $"{iff} FAMISTUDIO_EXP_EPSM_FM_CHN_CNT > {fm_idx}";
            }
            else // if (channel.IsEPSMRythmChannel)
            {
                return $"{iff} FAMISTUDIO_EXP_EPSM_RHYTHM_CHN{rhythm_idx}_ENABLE";
            }
        }
    }
